...
function updateNotesTable(noteId, titleNote) {
  var table = document.getElementById("notes-table");
  //this lines for delete exist rows when serching
  ...
  getNotes(titleNote)
    .then((data) => {
      data.forEach((note) => {
        ...
        cell4.innerHTML = `<a onClick="openEditModal('${note["_id"]}')" href="#"><img src="images/edit.png" style="width: 22px"></img></a>
...
// Function : searching by title
function searchNotes() {
  const serachTitle = document.getElementById("searchInput").value;
  updateNotesTable(undefined, serachTitle);
}
